...
- For multi-node deployment, I use Kubernetes which is deployed via Helm and reconciled using ArgoCD which automatically deploys in rolling update fashion by default to achieve near zero-downtime deployment. CD workflow, in GitHub Actions, is used to update image tag in Helm manifest file.
